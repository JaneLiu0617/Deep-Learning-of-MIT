...
def perceptron_single_step_update(
    ...
    # Your code here
    epsilon = 0
    if label * (np.dot(feature_vector, current_theta) + current_theta_0) <= epsilon:
...
def classify(feature_matrix, theta, theta_0):
    ...
    res = []
    for i in range(len(feature_matrix)):
        y = np.dot(feature_matrix[i], theta) + theta_0
        if y > epsilon:
            res.append(1)
        else:
            res.append(-1)
...
def classifier_accuracy(
    ...
    """
    # use perceptron algorithm to calculate theta and theta_0
    theta, theta_0 = classifier(train_feature_matrix, train_labels, **kwargs)
    # use the theta and theta_0 to predict the train and validation data
    train_preds = classify(train_feature_matrix, theta, theta_0)
    val_preds = classify(val_feature_matrix, theta, theta_0)
    # calculate the accuracy of train and validation
